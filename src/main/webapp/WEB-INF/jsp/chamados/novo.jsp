          ...
          <div class="col-md-8">
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Preencha os dados do chamado</h3>
              </div>

              <form method="post" action="<c:url value='/chamados/novo'/>" enctype="multipart/form-data">
                <div class="card-body">
                  ...
                  <div class="form-group">
                    <label for="responsavelId">Responsavel (opcional)</label>
                    <select class="form-control select2" id="responsavelId" name="responsavelId">
                      <option value="">Nao atribuir agora</option>
                      <c:if test="${empty colaboradores}">
                        <option value="" disabled>Nenhum colaborador ativo encontrado</option>
                      </c:if>
                      <c:forEach var="colaborador" items="${colaboradores}">
                        <option value="${colaborador.id}" ${chamado.responsavelId == colaborador.id ? 'selected' : ''}>
                          <c:choose>
                            <c:when test="${not empty colaborador.perfil and not empty colaborador.perfil.nomeCompleto}">
                              <c:out value="${colaborador.perfil.nomeCompleto}"/>
                            </c:when>
                            <c:otherwise>
                              <c:out value="${colaborador.username}"/>
                            </c:otherwise>
                          </c:choose>
                        </option>
                      </c:forEach>
                ...
                <div class="card-footer">
                  ...
                  <a href="<c:url value='/chamados/lista'/>" class="btn btn-secondary">Cancelar</a>
                </div>
              </form>
            </div>
          </div>
